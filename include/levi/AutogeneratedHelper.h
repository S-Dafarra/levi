...
private:
    ...
    void getLiteralExpression() {
        ...
        for (auto& final : m_finalExpressionIndices) {
            splitExpression(m_literalSubExpressions[final].literal, m_finalExpressions[final]); //add some newlines where needed
        }
...
public:
    ...
    const std::vector<SqueezedMatrixRef>& evaluateGenerics() {
        for (size_t generic : m_generics) {
            m_expandedExpression[generic].buffer = m_expandedExpression[generic].partialExpression.evaluate(false);
        }
