...
private:
    ...
    std::unordered_map<size_t, std::string> m_indexToLabelsMap;
...
public:
    ...
    MultipleCompiledExpressions(const std::vector<CompiledElement>& elements, const std::string& name) {

        std::vector<levi::ExpressionComponent<EvaluableT>> expressions;

        for (auto& element: elements) {
            m_indexToLabelsMap[expressions.size()] = element.first;
