...
private:
    ...
    using base_type = levi::CompiledEvaluable<SqueezedMatrix, typename EvaluableT::matrix_type>;
    shlibpp::SharedLibraryClass<base_type> m_compiledEvaluable;
...
public:
          ...
          , m_fullExpression(fullExpression)
    {
        ...
        shlibpp::SharedLibraryClassFactory<base_type> compiledEvaluableFactory;

        compiledEvaluableFactory.extendSearchPath(buildDir + "/lib");
        compiledEvaluableFactory.extendSearchPath(buildDir + "/lib/Release");

        bool isLibOpen = compiledEvaluableFactory.open(cleanName.c_str(), (cleanName + "Factory").c_str());

        assert(isLibOpen && compiledEvaluableFactory.isValid() && "Unable to open compiled shared library.");

        m_compiledEvaluable.open(compiledEvaluableFactory);

        assert(m_compiledEvaluable.isValid() && "The compiled instance is not valid.");
