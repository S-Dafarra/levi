...
public:
          ...
          , m_fullExpression(fullExpression)
    {
        ...
        zz::fs::FileEditor header(headerName, true);
        assert(header.is_open());

        header << "//This file has been autogenerated" << zz::os::endl();
        header << "#ifndef LEVI_COMPILED"<< cleanName << "_H" << zz::os::endl();
        header << "#define LEVI_COMPILED"<< cleanName << "_H" << zz::os::endl();

        header << "#include<levi/CompiledEvaluable.h>" << zz::os::endl() << zz::os::endl();
        header << "class " << cleanName << ": public levi::CompiledEvaluable<" << type_name<SqueezedMatrix>()
               << ", " << type_name<typename EvaluableT::matrix_type>() << "> {" <<zz::os::endl();
        header << "public:" << zz::os::endl();
        header << "    virtual void evaluate(const std::vector<" << type_name<SqueezedMatrix>() << ">& generics, "
               << type_name<typename EvaluableT::matrix_type>() << "& output) override;" << zz::os::endl();
        header << "};" << zz::os::endl();
        header << "#endif //LEVI_COMPILED"<< cleanName << "_H" << zz::os::endl();


        std::string cppName = workingDir + "/source.cpp";

        zz::fs::FileEditor cpp(cppName, true);
        assert(cpp.is_open());

        cpp << "//This file has been autogenerated" << zz::os::endl();
        cpp << "#include \"source.h\" " << zz::os::endl() << zz::os::endl();
        cpp << "void " << cleanName << "::evaluate(const std::vector<" << type_name<SqueezedMatrix>() << ">& generics, "
            << type_name<typename EvaluableT::matrix_type>() << "& output) {" << zz::os::endl();
        cpp << "    output = ";
        cpp << m_literalSubExpressions[0].literal << ";" << zz::os::endl() << "}" << zz::os::endl();
